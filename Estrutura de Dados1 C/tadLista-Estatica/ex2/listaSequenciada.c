...
int insere_lista_final(Lista* li, struct aluno al){
  ...
  if(li->qtd == (li->tamanho))//MAX 
     li = realocar_lista(li);
...
//-------------------REALOCAR------------------------
Lista* realocar_lista(Lista *li){
  int aux = (li->tamanho)*2;
  li =  realloc(li, aux*sizeof(Lista));
  return li;
}
...
int compactar_lista(Lista* li){
  ...
  int tamL, aux1;
  tamL = li->qtd;
  aux1 = ceil(tamL/100)*100;

  li = realloc(li, aux1*sizeof(Lista));
  return 0;
}